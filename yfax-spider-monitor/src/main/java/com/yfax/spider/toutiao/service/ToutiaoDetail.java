...
//		//默认返回数据为10条
    ...
    public void removeDetailInfo(String tag) throws UnknownHostException {
    	 	//获取开始时间
		long start=System.currentTimeMillis();  
		logger.info("tag=" + tag + "，===========================================================");
		logger.info("tag=" + tag + "，=============开始获取列表数据进行移除处理 start=================");
		...
		SearchHits hits = response.getHits();  
		logger.info("tag=" + tag + "，预计需清除数据总数：count=" + hits.getTotalHits());
		
		if(hits.getTotalHits() > 0) {
			for (int i = 0; i < hits.getTotalHits(); i++) {
				try {
					String id = hits.getAt(i).getSource().get("id").toString();
					String title = hits.getAt(i).getSource().get("title").toString();
					String gatherTime = hits.getAt(i).getSource().get("gatherTime").toString();
					String articleDate = DateUtil.stampToDate(Long.valueOf(gatherTime)/1000);
					
					logger.info("---------------");
					logger.info("id=" + id + ", title=" + title);
					logger.info(",gatherTime=" + gatherTime + ", dateOfGatherTime=" + articleDate);
					long[] times = DateUtil.getDistanceTimes(DateUtil.getCurrentLongDateTime(), articleDate);
					logger.info("day=" + times[0] + ", hour=" + times[1] + ", min=" + times[2] + ", sec=" + times[3]);
					
					//内容缺失一天以上的文章，清除 / 内容缺失两个小时以上的文章，清除
					if(times[0] > 0 || times[1] > 2) {
						DeleteResponse result = EsService.getClient().prepareDelete("commons", "webpage", id).execute().actionGet();
						RestStatus status = result.status();
						logger.info("tag=" + tag + "，status = " + status);
					}
				} catch (Exception e) {
					logger.error("文章数据异常：" + e.getMessage());
				}
			}
		}else {
			logger.info("tag=" + tag + "，无缺失内容的文章，跳过处理。");
		}
		
		//获取结束时间
		long end=System.currentTimeMillis();
		logger.info("tag=" + tag + "，详情数据程序运行时间： " + (end-start) + " ms");
		logger.info("tag=" + tag + "，=============开始获取列表数据进行移除处理 end=================");
		logger.info("tag=" + tag + "，===========================================================");
    }
